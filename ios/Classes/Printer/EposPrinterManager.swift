...
/// Contract
/// * Channel Name: epson_pos_printer/printer
...
///
/// * Method: void addCommand({id: String, args: {Uint8List data}})
///
/// * Channel Name: epson_pos_printer/printer/\(id)/status
...
public class EposPrinterManager : NSObject, FlutterPlugin {
    @objc
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "epson_pos_printer/printer", binaryMessenger: registrar.messenger())
        let instance = EposPrinterManager(channel: channel, messenger: registrar.messenger())
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "addCommand":
            runHandler(addCommand, for: call, returnTo: result)
    ...
    func createPrinter(_ arguments: Any?) throws -> Int32 {
        guard let argMap = arguments as? Dictionary<String, Any>,
              let seriesName = argMap["series"] as? Int32,
              let modelName = argMap["model"] as? Int32 else {
    ...
    /// * Method: void addCommand({id: String, args: {Uint8List data}})
    func addCommand(_ arguments: Any?) throws -> Int32 {
        let (printer, args) = try instArgsDict(from: arguments)
        ...
        let code = printer.addCommand(data.data)
